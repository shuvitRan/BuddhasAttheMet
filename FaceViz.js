const margin = {
  ...
  let pageOnColor="rgb(252, 58, 58,0)";
  let pageOnDeco="3px solid rgb(255, 0, 0)";
  let pageOnTextColor="rgb(255, 0, 0)";

  let pageOffTextColor="rgb(0, 0, 0)";
  let pageOffColor="rgba(123, 122, 124, 0)";
  // let pageOffDeco="1px solid rgb(20, 20, 20,1)"
  let pageOffDeco="3px solid rgb(255, 255, 255,0)"
  // let my
  let loadingData;
  let dataReady= false;
...
Promise.all(promises).then((data) => {
    ...
    for (let i = 0; i <cultureDisplayName.length; i++) {
      ...
      // ThirdCV(cultureDisplayName[i], data);
    }
    loadingData = data;
    dataReady=true;
...
d3.selectAll(".grid-item").style("background-color","rgb(255, 255, 255)");
  ...
  .style("color", pageOffTextColor)
  .style("border-bottom",pageOffDeco);
  
    // .style("background-color",activeButtonColor);
    d3.select("#page2")
    .style("background-color", pageOnColor)
    .style("color", pageOnTextColor)
    .style("border-bottom",pageOnDeco);
